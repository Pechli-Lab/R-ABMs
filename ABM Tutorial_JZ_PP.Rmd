...
ABM <- function(df_X, M_init, seed = 1, n_cycles = 10) {
  ...
  for (t in 1:n_cycles) {
    ...
    df_X <- list_quarantine$df_X
    
    # Update the vector of health states (m_M[, t+1]) for the next cycle
    # Ensures deterministic state transitions based on the updated quarantine checks.
    m_M[, t+1] <- list_quarantine$M_t
