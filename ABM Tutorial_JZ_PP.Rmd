...
We developed an agent based model where we assumed a daily cycle length. We modelled a hypothetical environment where individuals will go to work and can be infected at work and PP: is that still true?in the community but cannot infect others at home. We used the model to prospectively estimate a 60 day period. We assumed a latency of 4 days from acquiring the infection until symptom development.
...
ABM <- function(df_X, M_init, seed = 1, n_cycles = 10) {
  ...
  for (t in 1:n_cycles) {
    ...
    df_X            <- list_quarantine$df_X  #Update quarantine clock t_iso PP: this is not doing what the comment says
