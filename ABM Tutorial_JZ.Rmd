...
Quarantine_Check <- function(df_X, M_t){
  ...
  # find people in quarantine exclude those need to be reset (df_X$v_Ti == t_detect & df_X$t_iso > 0)
...
fun_work_exposure <- function(df_X, M_t) {
  ...
  ## PP: give a more inttuitive name
  
  v_atwork <- df_X$t_iso == 0 #M_t == "S" | M_t == "I"  | M_t == "R"     # Filter workers currently at work df_X$t_iso == 0
  df_atwork <- df_X[v_atwork,]
  
  v_exposure <- df_X[M_t  == "S",]$n_exposure
  v_infectious <- df_X[M_t == "I","v_Ti"] > t_latent
