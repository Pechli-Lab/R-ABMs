...
In this section we define functions that will be used by the ABM. We try to modularize functions, so that debugging becomes eaiser.

The function `fun_infect_prob` calculates the infection probability by every cycle, assuming the probability increases by the number of exposures assuming these exposures are independent of each other.
```{r}
...
We developed an agent based model where we assumed a daily cycle length. We modelled a hypothetical environment where individuals will go to work and can be infected at work and in the community but cannot infect others at home. We used the model to prospectively estimate a 60 day period. We assumed a latency of 4 days from acquiring the infection until symptom development.
...
We ran the model for 100 days and we generate a hypothetical population of 10,000 individuals. We assume 4 days ofincubation period and another 5 days until one is detected and placed in isolation. we also further assumed that the period unctil recovery is 14 days. We further assume that 30% of the cohort starts as infected and that there is an 8% chance of infection daily and a 2% probability per interaction with an infected individual at work.
...
#PP: JZ can you describe what each line does?
...
#PP : JZ can you add more detials here?
...
Probs <- function(df_X, M_t, n_cycles) { 
  ...
  v_exposure             <- df_X[M_t == "S",]$n_exposure      # how many times someone was exposed to an infected individual
  ...
  m_p_t["I",  M_t == "S" ] <-     p_Sus_Inf     # Probability for susceptible individuals to become infected (I)
  ...
  check_transition_probability(t(m_p_t), verbose = TRUE)
  check_sum_of_transition_array(t(m_p_t), n_rows = n_i, n_cycles = n_cycles, verbose = TRUE)
...
fun_Quarantine_Check <- function(df_X, M_t){
  ...
  M_t[v_release & M_t == "IQ"] <- "R" # Infected individuals move to recovered state   #PP: If someone is pst the 14 days but still in quarantine? are they in the IQ state?
...
ABM <- function(df_X, M_init, seed = 1, n_cycles = 10) {
  ...
  for (t in 1:n_cycles) {
    ...
    df_X            <- list_quarantine$df_X  #Update quarantine clock t_iso for df_X PP: this is not doing what the comment says
