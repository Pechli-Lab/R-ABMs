...
detect_time                 <- 9                 # # becoming infetious to isolation d
...
# atwork \ university
p_TransmitWork        <- 0.02 # atwork transmission probability per interaction

# Determine the initial number of 'S' (susceptible) and 'I' (infected) agents based on seed_ratio
seed_ratio <- 0.3
n_S <- round((1 - seed_ratio) * n_i)
n_I <- n_i - n_S
```
...
# Create agent matrix
df_X <- data.frame(
  ptid = 1:n_i,  # Assign individual IDs
  Sex = sample(x = c("Female", "Male"), prob = c(0.5, 0.5), size = n_i, replace = TRUE),  # Random gender assignment
  Age = round(rnorm(n_i, mean=40, sd=10), digits = 0),  # Age drawn from a normal distribution
  wid = 0,  # Workplace ID
...
df_X <- df_X %>%
  mutate(
    ...
    atwork = TRUE,         # Indicates if the agent is at work # at quarantine people can't go to work
    v_Ts = 0,              # Time since being infected
    detect_time = detect_time, 
  )

# Set the number of contacts at work based on workplace size
work_in_sample <- df_X$ptid[which(df_X$atwork == TRUE)]  # check at quarantine and at work 
work_size <- table(df_X[work_in_sample, ]$wid)
work_info <- data.frame(wid = as.numeric(as.character(names(work_size))), wsize = as.numeric(as.matrix(work_size)))
work_place_size <- merge(df_X[work_in_sample, ], work_info, by = "wid") %>%
  arrange(ptid) %>%
  rowwise() %>%
  mutate(contact_num = sample(0:(wsize - 1), 1))  # Randomly assign contact number

df_X$contact_num[work_in_sample] <- work_place_size$contact_num
...
Probs <- function(M_t, df_X) { 
  ...
  sus_prob_home = p_TransmitCommunity  # if it is probability, use p_sus
  
  sus_prob = rep(0,nrow(sus))
  at_work = sus$contact_type == 1 # remove contact_type everyone is at work.
  if(sum(at_work) > 0){
    work_infect = infect_prob(p_TransmitWork,sus[at_work,]$expl)  # if it is probability, use p_. # use num_contacts instead of expl
    sus_prob[sus$expl > 0 & sus$contact_type == 1] = work_infect # if it is probability, use p_.
  }
  # p_work_infect = infect_prob(p_TransmitWork,sus$expl)
  
  p_Sus_Inf <- (1- ((1-  p_TransmitCommunity ) * (1 - sus_prob))) # p_work_infect  1 minus the prob of being safe all day
  ...
  sus_prob_home_Q  <- rep(p_TransmitCommunity, nrow(sus_Q)) # try to turn it off
...
Quarantine_Check <- function(pop, pre_state){
  ...
  full_q                     = pop$qtime == q_maxtime # & pop$state != "R" # & !reset_people # if you've been recovered you are exempt from quarantine only do quarantine once.
  
  #find susceptible people and infected people who meet quarantine time
  sus_full_q                 = full_q & pop$state == "SQ"
  inf_full_q                 = full_q & pop$state == "IQ"
  
  # condition 3: find people who stay in quarantine and the quarantine clock has to be incremented.
  else_q                     = # !full_q            #pop$qtime < q_maxtime # & pop$qtime >0) #& !reset_people & pop$state != "R"  
  ...
  new_infected_workplace        = unique(pop$wid[new_infected]) #families have newly infected in quarantine cases
  ...
  new_qurantine              = (((pop$wid %in% new_infected_workplace & pop$qtime == 0) | #(pop$wid %in% new_infected_work & pop$qtime == 0) |
                                   (pop$wid %in% reset_workplace)) & (pop$state != "R") & (!(pop$essential == 1 & pop$getspsl == 0))  ) #people whose quarantine clocks have to be set to 1 including
...
work_exposure <- function(pop) {
  ...
  #      'state', 'wid', 'ptid', and others that are relevant to the exposure at work.
  # Returns:
...
Detection <- function(pop, v_TS){
  ...
  detected_individuals = (v_TS == detect_time) & pop$state == "I"  # we dont' need pop$detect_time
...
MicroSim <- function(df_X, seed = 1, n_cycles = 10) {
  ...
  for (t in 1:n_cycles) {
    ...
    v_detected_daily[t] =  sum((v_Ts == df_X$detect_time) & prob_new_state != "R")
