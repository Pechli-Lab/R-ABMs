...
```{r}
# Calculate the infection probability by every cycle
...
Quarantine_Check <- function(pop, pre_state){
  ...
  full_q                     = pop$qtime == q_maxtime & !reset_people & pop$state != "R" 
  ...
  else_q                     = (pop$qtime < q_maxtime & pop$qtime >0) & !reset_people & pop$state != "R"  
...
Detection <- function(pop, v_TS){
  ...
  detected_individuals = (v_TS == pop$detect_time) & pop$state != "R"
