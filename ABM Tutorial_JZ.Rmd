...
age_mean                 <- 40                          # Mean age 
age_sd                   <- 10                          # Standard deviation of age
p_male                   <- 0.5                         # Proportion of male
age_lim                  <- 18                          # Minimum age
...
df_X$Age[df_X$Age < age_lim] <- age_lim # only adults are included in the simulation
...
df_X$t_infected[M_init == "I"] = sample(c(1:9),n_I,replace = T)
...
```{r}
fun_Probs <- function(df_X, M_t, n_cycles) { 
...
fun_work_exposure <- function(df_X, M_t) {
  ...
  v_agents_atwork        <- df_X$t_iso == 0   # if not in isolation, by default a worker
  # Create a subset of agents at work
...
MicroSim <- function(df_X, M_init, seed = 1, n_cycles = 10) {
  ...
  for (t in 1:n_cycles) {
    ...
    # Update vector with new infection and total infection counts.
