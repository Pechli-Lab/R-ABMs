...
Quarantine_Check <- function(df_X){
  # Arguments:
  ...
  v_release_SQ     <- v_release_S & df_X == "SQ"
  v_release_IQ     <- v_release_I & df_X == "IQ"
    
  # condition 3: find people who stay in quarantine and the quarantine clock has to be incremented.
  
   
  v_in_iso <- df_X$t_iso < t_iso_max & df_X$t_iso > 0
  
  
  # condition 4: find newly infected people entering quarantine NOTE: people enter quarantine by detection
  v_new_detected                <-  which(df_X$v_Ti == t_detect ) #new infected generated from infected
  
  # condition t_iso_max: find newly infected people's dependencies in families and workpalces
  v_new_detected_workplace     = unique(df_X$wid[v_new_detected]) #families have newly infected in quarantine cases
  
  v_new_iso    <- (df_X$wid %in% v_new_detected_workplace & df_X$state != "R") #
  
  v_increment <- v_new_iso | v_in_iso
  
  df_X$t_iso[v_increment] <- df_X$t_iso[v_increment] + 1
  
  df_X$state[v_new_iso] <-  paste(pop$state[v_new_iso],"Q",sep = "")
  ...
  df_X$state[v_release_SQ]        <-  "S" #when quarantine ends, state goes back to susceptible
  df_X$state[v_release_IQ]        <-  "R" #when quarantine ends, state goes back to recovered
  ...
  people whose quarantine clocks have to be set to 1 including
  exist_quarantine <- df_X$t_iso  >0 & df_X$t_iso < t_iso_max
  pop$t_iso[new_qurantine]          = pop$t_iso[new_qurantine] + 1
  pop$t_iso[exist_qurantine]        = pop$t_iso[new_qurantine] + 1
    # -------------------------------------
  ...
  M_t[df_X$v_Ti == t_detect  & M_t =="I" ] <- "IQ"
  M_t[df_X$v_Ti == t_detect  & M_t =="IQ" ] <- "IQ"
...
Detection <- function(pop, v_Ti){
  ...
  # v_Ti: A vector representing time since onset of illness for each individual in 'pop'.
  # Returns:
  ...
  detected_individuals = (v_Ti == t_detect) & pop$state == "I"  # we dont' need pop$t_detect
