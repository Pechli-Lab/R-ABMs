...
detect_time                 <- 9                        # becoming infetious to isolation
...
seed_ratio <- 0.3                                       # proportion of seed infected people when initializing the simulation population  
n_S <- round((1 - seed_ratio) * n_i)
n_I <- n_i - n_S


# workplace characteristics 
work_size <- c(5, 6, 7)                                 # we assume workplaces with sizes of 5 to 7 people
...
df_X <- data.frame(
  ...
  Sex  = sample(x = c("Female", "Male"), 
                ...
                size = n_i, replace = TRUE),        # Random gender assignment
  Age = round(rnorm(n_i, 
                    mean = age_mean,
                    sd = age_sd),
              digits = 0),                    # Age drawn from a normal distribution
  wid   = 0,  # Workplace ID
