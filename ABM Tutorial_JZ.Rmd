...
fun_Quarantine_Check <- function(df_X, M_t){
  ...
  ## PP comment: describe conditions similar to " if agents are already in quarantine "IQ", quarantine clock need to be reset"
    
  v_in_iso <- (df_X$t_iso < t_iso_max & df_X$t_iso > 0 & df_X$t_infected != t_detect)
  
  # find people in quarantine exclude those need to be reset (df_X$t_infected == t_detect & df_X$t_iso > 0)
  ...
  v_new_detected_in_q                <-  df_X$t_infected == t_detect & df_X$t_iso > 0
  
  # if agents are in I state "I" and meet the t_detect, all coworkers (except recovered) will be put into quarantine.
  
  v_new_detected_out_q               <-  df_X$t_infected == t_detect & df_X$t_iso == 0
  
  # find newly infected agents' dependencies in  workpalces
  
  v_new_detected_workplace           <- unique(df_X$wid[v_new_detected_out_q]) # workplace have newly infected in quarantine cases
  v_new_iso                          <- (df_X$wid %in% v_new_detected_workplace) & M_t != "R"
  
  # find agents' quarantine clock need to be incremented
  ## PP comments: newly detected or previously isolated, try to explain logical statements
  ...
  M_t[v_release & M_t == "SQ"]                  <-  "S" #when quarantine ends, state goes back to susceptible
  M_t[v_release & M_t == "IQ"]                  <-  "R" #when quarantine ends, state goes back to recovered
  
  
  # find susceptible people and infected people who meet quarantine time
  ##PP comments do the same for above as   M_t[v_new_iso & M_t == "S"]        <- "SQ"
...
fun_work_exposure <- function(df_X, M_t) {
  ...
  v_agents_atwork <- df_X$t_iso == 0   # Filter workers currently at work "S","I","R"
  
  df_atwork <- df_X[v_agents_atwork,] # subset of agents, who participate interaction: at work COVID exposure
  
  v_n_exposure <- df_atwork[M_t  == "S",]$n_exposure # vector of number of expousre in current cycle
  v_infectious <- df_atwork$t_infected > t_latent # vector of infectious agents in at work subset df.
  
  v_interact_wid <- df_atwork$wid
  v_pid_S    <-    df_X[M_t == "S","pid"]
  ...
  v_inf_workplace <- df_atwork[v_infectious, "wid"]                                # List of workplaces with infectious workers
...
MicroSim <- function(df_X, seed = 1, n_cycles = 10) {
  ...
  m_M[, 1] <- df_X$state
  m_Ti[,1] <- df_X$t_infected
  
  t_infected <- df_X$t_infected    
  v_inf <- rep(0, n_i)
  ...
  for (t in 1:n_cycles) {
    ...
    t_infected <- if_else(qc_state %in% all_infect_states, t_infected + 1, 0)
    m_Ti[,t + 1] = t_infected

    # Update individuals who have reached recovery time to 'R' state.
    v_go_to_recover <- if_else(t_infected >= t_recover, TRUE, FALSE)
