...
work_size <- c(5, 6, 7) # we assume workplaces with sizes of 5 to 7 people. this could be also a work group inside a larger workplace
...
df_X <- data.frame(
  ...
  Sex  = sample(x = c("Female", "Male"), prob = c(p_male,1 - p_male), size = n_i, replace = TRUE),  # Random gender assignment
  Age = round(rnorm(n_i, mean = age_mean, sd = age_sd), digits = 0),  # Age drawn from a normal distribution
  wid   = 0,  # Workplace ID
...
#PP lets asses if you indeed need all of  those. 
...
work_size <- table(df_X$wid)
work_info <- data.frame(wid = as.numeric(names(work_size)), wsize = as.matrix(work_size))
df_X             <- merge(df_X, work_info, by = "wid")
df_X$contact_num <- sample(0:(df_X$wsize - 1), 1) #Randomly assign contact number
...
Probs <- function(M_t, df_X) { 
  ...
  df_X_sus <- df_X[df_X$state == "S",] #identify those susceptible
  
     p_sus <- infect_prob(p_TransmitWork, df_X_sus$expl) # if it is probability, use p_.

  p_Sus_Inf <- (1- ((1-  p_TransmitCommunity ) * (1 - p_sus))) # p_work_infect  1 minus the prob of being safe all day
...
Quarantine_Check <- function(df_pop){
  ...
  full_q      <- df_pop$qtime == q_maxtime # & pop$state != "R" # & !reset_people # if you've been recovered you are exempt from quarantine only do quarantine once.
  ...
  new_infected               = which(df_pop$v_ti == detect_time ) #new infected generated from infected
  ...
  new_qurantine   <- ((df_pop$wid %in% new_infected_workplace & df_pop$qtime == 0) ) #people whose quarantine clocks have to be set to 1 including
