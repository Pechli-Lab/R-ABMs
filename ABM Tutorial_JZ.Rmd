...
n_S <- round((1 - p_seed_ratio) * n_i)                  # number of initialized susceptible agents
  ...
  if (sum(pop$atwork == 1) > 0) {
    ...
    if (nrow(inf_worker) > 0) {
      ...
      if (length(contact) != 0) {
        ...
        expl_list[select_pop$pid %in% contact_table$pid] <- expl_list[select_pop$pid %in% contact_table$pid] + contact_table$expl
